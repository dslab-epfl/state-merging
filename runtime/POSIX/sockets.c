...
exe_sockaddr_t *__allocate_sockaddr(int domain, exe_sockaddr_t *a) {
  ...
  switch (domain) {
  case PF_INET:
    a->addr = calloc(1, sizeof(struct sockaddr_in));
  ...
  case PF_INET6:
    a->addr = calloc(1, sizeof(struct sockaddr_in6));
    if (!a->addr) return NULL;
...
int __fd_socket(unsigned long *args) {
  ...
  case PF_INET6:
    ...
    default:
      klee_warning("Only SOCK_STREAM and SOCK_DGRAM sockets supported at this time.");
  ...
  default:
    ...
    klee_warning("Only PF_INET and PF_INET6 sockets supported at this time.");
...
int __fd_bind(unsigned long *args) {
  ...
  if (f->dfile) {
    ...
    klee_warning("XXX: symbolic file descriptor\n");
    assert(f->local.addr);
    ...
    klee_warning("XXX: concrete file descriptor\n");
    args[0] = f->fd;
...
int __fd_getsockname(unsigned long *args)
{
  ...
  if (f->dfile) {
    ...
    *addrlen = f->local.addrlen;
    ...
    /* concrete file descriptor */
    args[0] = f->fd;
    os_r = syscall(__NR_socket, SYS_GETSOCKNAME, args);
    args[0] = sockfd;
    if (os_r < 0) {
      errno = klee_get_errno();
      return -1;
...
int __fd_getpeername(unsigned long *args)
{
  ...
  if (f->dfile) {
    ...
    *addrlen = f->foreign->addrlen;
    ...
    /* concrete file descriptor */
    args[0] = f->fd;
    os_r = syscall(__NR_socket, SYS_GETPEERNAME, args);
    args[0] = sockfd;
    if (os_r < 0) {
      errno = klee_get_errno();
      return -1;
